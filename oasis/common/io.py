...
def create_initial_folders(folder, restart_folder, sys_comp, tstep, info_red,
    ...
    for ui in comps:
        ...
        tstepfiles[ui] = TimeSeries(path.join(tstepfolder, ui + '_from_tstep_{}'.format(tstep)))
...
def save_tstep_solution_h5(tstep, q_, u_, newfolder, tstepfiles, constrained_domain,
                           output_timeseries_as_vector, u_components, AssignedVectorFunction,
                           scalar_components, NS_parameters):
    """Store solution on current timestep to TimeSeries file."""
    ...
    if output_timeseries_as_vector:
        ...
        for comp, tstepfile in six.iteritems(tstepfiles):
            if comp == "u":
                ...
                # tstepfile.store(q_[comp].vector(), tstep)

            else:
                tstepfile.write(tstepfile.function, float(tstep))
                # tstepfile.store(tstepfile.function, tstep)

    else:
        for comp, tstepfile in tstepfiles.items():
            if comp == "omega":
                break
                # tstepfile.store(u_.vector(), tstep)
            tstepfile.store(q_[comp].vector(), float(tstep))
...
def save_checkpoint_solution_h5(tstep, q_, q_1, newfolder, u_components,
    ...
    for ui in q_:
        ...
        # h5file = path.join(checkpointfolder, ui + '.xdmf')
        oldfile = path.join(checkpointfolder, ui + '_old.h5')
        # oldfile = path.join(checkpointfolder, ui + '_old.xdmf')
        # For safety reasons...
        ...
        # newfile = XDMFFile(MPI.comm_world, h5file)
        # newfile.parameters["rewrite_function_mesh"] = False
        # newfile.parameters["flush_output"] = True
        newfile.flush()
        newfile.write(q_[ui].vector(), '/current')
        # newfile.write_checkpoint(q_[ui])
        if ui in u_components:
            newfile.write(q_1[ui].vector(), '/previous')
            # newfile.write_checkpoint(q_1[ui])
